...
export default function Note({ title, children }: Props) {
	return (
		...
		>
			<div data-slot="alert-title" className="col-start-2 line-clamp-1 min-h-4 font-medium tracking-tight">{title}</div>
			<div data-slot="alert-description" className="inline col-start-2 grid justify-items-start gap-1 text-sm [&amp;_p]:leading-relaxed text-card-foreground/80">
				{children}
			</div>
		</div>
	)
